...
$conn = mysqli_connect('localhost', 'root','', 'munkahely') or die("Hibás csatlakozás!");
    ...
    echo '<h2>Pihenő törlése</h2>';

    if(mysqli_select_db($conn, 'munkahely')) {
                    $sql = "SELECT `SzobaID`, `Fotel`, `Kanapé`, `Kávéfőző` FROM pihenő";
                	...
                	while(($current_row = mysqli_fetch_assoc($res))!=null) {
                        ...
                        echo '<td>
                        <form method="POST" action="DeletePiheno.php">
                            <input type="hidden" name="sor" value="'.$current_row["SzobaID"].'"/>
                            <input type="submit" name="submit" value="Pihenő törlése" />
                        </form></td>';
                        echo '</tr>';
                	}
                    echo '</table>';
